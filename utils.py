...
def read_data_sets(train_dir,
  ...
  train_images = extract_pnt(PNT_PATH_TRAIN)
  train_labels = extract_labels(PNT_PATH_TRAIN, one_hot=one_hot)
  test_images = extract_pnt(PNT_PATH_TEST)
  test_labels = extract_labels(PNT_PATH_TEST, one_hot=one_hot)
  ...
  for file in PNTfiles:
      if not os.path.isdir(file): #判断是否是文件夹，不是文件夹才打开  
         with open(path+"/"+file,'rb') as f:
          ...
          images = data.reshape(SmpLen, rows, cols, 1)
